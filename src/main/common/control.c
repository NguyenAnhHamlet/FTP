               ...
               endpoint_type type)
{
    ...
    case CLIENT:
    {
        ...
        {
            LOG("Fail to change dir\n");
            operation_abort(c_channel);
            return 0;
        }

        LOG("Successfully changed dir\n");
                ...
                endpoint_type type)
{
    ...
    case CLIENT:
    {
        ...
        if(!control_channel_send(c_channel) ||
           !control_channel_read_expect(c_channel, SUCCESS))
        {
            LOG("Failed to change mode\n");
            operation_abort(c_channel);

            return 0;
        }

        LOG("Done change mode\n");
    ...
    case SERVER:
    {
        ...
        {
            LOG("Fail to change mode\n");
            operation_abort(c_channel);

            return 0;
        }

        LOG("Change mode file done\n");
        control_channel_append_ftp_type(SUCCESS, c_channel);
        control_channel_send(c_channel);
                ...
                unsigned int n_len, endpoint_type type)
{
    ...
    case CLIENT:
    {
        ...
        if(!control_channel_send(c_channel) ||
           !control_channel_read_expect(c_channel, SUCCESS))
        {
            LOG("Delete remote file failed\n");
            operation_abort(c_channel);

            return 0;
        }

        LOG("Delete remote file done\n");
        break;
    }
    case SERVER:
    {
                    ...
                    char* res, unsigned int* r_len, endpoint_type type)
{
    ...
    case CLIENT:
    {
        ...
        if(!control_channel_send(c_channel) ||
           !control_channel_read_expect(c_channel, DIR))
        {
            LOG("Failed to list dir\n");
            operation_abort(c_channel);
    ...
    case SERVER:
    {
        ...
        int r_len;
        if(!control_channel_read_expect(c_channel, DIR))
        {
            LOG("Unknown option\n");
            operation_abort(c_channel);
                    ...
                    endpoint_type type)
{
    ...
    case CLIENT:
    {
        control_channel_set_header(c_channel, 0, sizeof(Packet), 
                                   0, IDLE, 0);
        control_channel_append_int(time_out);
        if(!control_channel_send(c_channel) || 
           !control_channel_read_expect(c_channel, SUCCESS))
        {
            LOG("Set timeout remote failed\n");
            operation_abort(c_channel);
                   ...
                   unsigned int* m_len)
{
    ...
    case CLIENT :
    {
        ...
        {
            LOG("Could not get mod time of remote file\n");
            operation_abort(c_channel);
                    ...
                    unsigned int* file_size, endpoint_type type)
{
    ...
    case SERVER:
    {
        ...
        control_channel_set_header(c_channel, 0, sizeof(Packet), 0, SIZE, 0);
        control_channel_append_int(st.st_size, c_channel);
                       ...
                       char* update_name, int u_len, endpoint_type type)
{
    ...
    case CLIENT:
    {
        ...
        if(!control_channel_send(c_channel) || 
           !control_channel_read_expect(c_channel, SUCCESS))
        {
            LOG("Fail operation change remote server file name\n");
            return 0;
        }
    ...
    case SERVER:
    {
        ...
        control_channel_get_str(c_channel, file_name, &n_len);
        for(update_name = file_name, *update_name != ' '; update_name++ );
        update_name = '\0';
        update_name++;
